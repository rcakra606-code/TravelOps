...
export async function createApp() {
  ...
  app.post('/api/refresh', authMiddleware(false), async (req,res)=>{
    ...
    try { const decoded = jwt.verify(token, SECRET); const newToken = jwt.sign(decoded, SECRET, { expiresIn:'8h' }); res.json({ token:newToken }); } catch { res.status(403).json({ error:'Token expired' }); }
  });
